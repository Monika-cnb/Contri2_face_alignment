# just reviewing code to complete for face alignment - for control dataset - specific
...
training_data_path = initial_data_path + "Compressed/Controls/"

# path for saving the aligned files
output_dir = "./../../Desktop/Contri_2_updated/0_face_alignment/"
output_dir_path_train = output_dir + "0_aligned_dataset/0_bdi/0_control_comp/"
# code to read the patient dataframe
control_df = pd.read_csv(output_dir + "0_aligned_dataset/0_bdi/Control_file_comp.csv")
# print(patient_df)

error_files = []
video_num = 0
# code to read each row of the above dataframe
for val in range(len(control_df)):
    try:
        each_video = control_df['File'][val] + ".avi"
        vid_time_dur = control_df['interview'][val]
